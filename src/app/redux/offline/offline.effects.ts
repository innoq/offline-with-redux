...
export class OfflineEffects {
  sendStoredRequests$: Observable<Action> = createEffect(() =>
    this.actions$.pipe(
  ...
  private _resendRequests$ = createEffect(() =>
    this.actions$.pipe(
      ofType(startSynchingRequests),
      concatLatestFrom(() =>
        this.store.select((state: AppState) => state.offline.requests)
      ),
      tap(([_action, requests]) => {
        requests.map((request: OfflineRequest) => {
          this.offlineService
            .resendRequest(request.request)
            .pipe(
              tap((todo: Todo) => {
                ...
                todo.id = id.toString();
                this.store.dispatch(
                  replaceOfflineTodo({ id: request.id, todo })
                );
